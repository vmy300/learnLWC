...
export default class toDoManager extends LightningElement {
  ...
  connectedCallback() {
    ...
    //registering to pusub events
    registerListener('updatePubsub',this.updateTodoHandler,this);
    registerListener('deletePubsub',this.deleteTodoHandler,this);
    ...
    addTodo({ payload: JSON.stringify(todo) })
      .then(result => {
        if (result) {
          ...
          //we can pass payload as follows
          var eventParam={payload:JSON.stringify(todo)};
          //firing addEvent using pubsub
          fireEvent(this.pageRef,'addPubsub',eventParam); 
        }
      })
      .catch(error => {
  ...
  updateTodoHandler(event) {
    if (event) {
      console.log('Updated using pubsub');
      this.fetchTodos();
    }
  }
  ...
  deleteTodoHandler(event) {
    if (event) {
      console.log('Deleted using pubsub');
      this.fetchTodos();
    }
  }
